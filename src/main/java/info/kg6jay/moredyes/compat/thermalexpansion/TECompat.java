...
public class TECompat
{
	public static void registerThermalExpansion()
	{
		...
		for(int i=0;i<MDBlock.colors.length;i++)
		{
			...
			GameRegistry.registerBlock(MDBlock.rockwool[i],MDItemBlockColored.class,((MDBlockColored) MDBlock.rockwool[i]).getColorSet()+"MixRockwool");
			for(int a=0;a<=((MDBlockColored)MDBlock.rockwool[i]).getMaxMeta();a++)
			{
				...
				CraftingManager.getInstance().getRecipeList().add(new ShapelessOreRecipe(new ItemStack(MDBlock.rockwool[i],1,a),new ItemStack(MDItem.dye[i],1,a),"blockClothRock"));
			}
		}
	}
}
